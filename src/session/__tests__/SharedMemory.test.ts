...
describe('SharedMemory', () => {
  it('appends a timestamped one-line note', async () => {
    ...
    expect(mkdirs[0]).toMatch(/[\\/]ws[\\/]\.roam[\\/]memory$/);
    expect(appends).toHaveLength(1);
    expect(appends[0].file).toBe('/ws/.roam/memory/notes.md');
  ...
  it('loads empty string when the file is missing or unreadable', async () => {
    const sm = new SharedMemory('/ws/.roam/memory/notes.md', async () => { throw new Error('ENOENT'); });
  ...
  it('returns false (not throw) when append IO fails, true on success', async () => {
    ...
    const ok = new SharedMemory('/ws/.roam/memory/notes.md', async () => '', async () => undefined, async () => undefined);
    await expect(ok.append('agent-a', 'note')).resolves.toBe(true);
  });

  it('builds the memory path under .roam/memory', () => {
    expect(memoryFilePath('/ws')).toMatch(/[\\/]ws[\\/]\.roam[\\/]memory[\\/]notes\.md$/);
  });
